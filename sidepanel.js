...
function isSettingsOpen() {
  return app.dataset.settings === "open";
}

function setSettingsOpen(open) {
  app.dataset.settings = open ? "open" : "closed";
  settingsIcon.textContent = open ? "✕" : "⚙";
}
...
async function refreshScheduleView() {
  ...
  jobList.innerHTML = jobs.length ? jobs.map((j) => jobItemTemplate(j, webhooks)).join("") : "";
  ...
  qaList.innerHTML = quickActions
    .map((qa, i) => `
      <div class="formRow" data-qa="${i}">
        <div class="threeCol">
          <input class="qaLabel input" placeholder="Label" value="${escapeHtml(qa.label)}" />
...
function openSettings() {
  prevMode = app.dataset.mode || "message";
...
sAdd.addEventListener("click", async () => {
  ...
  if (!text) return setStatus(sStatus, "Message is empty.");
  if (delaySeconds < 10) return setStatus(sStatus, "Delay must be at least 10 seconds.");
  ...
  if (!r.ok) return setStatus(sStatus, r.reason);
...
// refresh remaining times
setInterval(() => refreshScheduleView(), 1000);
...
(async function init() {
  ...
  await setQuickActions(quickActions);
